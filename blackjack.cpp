//TO-DO LIST
//1. Implement a way to decide when an Ace is either an 11 or a 1.
...
//2. Create a vector of arrays to hold ascii art for each card. Display card images instead of card names.
...
int main() {
	...
	cout << "\nEnter any key to play, type EXIT to close the program...";
	cin >> playAgain;

	//USE RESPONSE INPUT TO EXIT THE GAME OR START THE GAME
	if (playAgain == "EXIT" || playAgain == "exit") {
		exitProgram();
	}
	else {
...
void letsPlay() {
	...
	//WHEN THE DRAWCARD() FUNCTION IS CALLED, THE APPROPRIATE VARIABLE(HAND) IS HANDED TO THE FUNCTION TO UPDATE THE VARIABLE.
	int dealerHand = 0;
	int playerHand = 0;
	...
	//DISPLAY EACH PLAYER'S HAND TOTAL
	...
	do {
		...
		//CONVERT THE STRING TO AN INTEGER SO THAT IT IS COMPATIBLE WITH THE SWITCH STATEMENT BELOW.
		int choice = stoi(actionChoice);

		//EXECUTE THE CHOICE
		switch (choice) {
		case 1:	//DISPLAY WHAT THE PLAYER DREW AND DISPLAY HAND TOTALS
				cout << "\nYou drew: ";
				cout << drawCard(deckOfCards_face, deckOfCards_value, size, playerHand) << endl;
				cout << "\nDealer has " << dealerHand << " + [Facedown card]" << endl;
				cout << "You have: " << playerHand << endl;
		...
		case 2: //DEALER REVEALS FACEDOWN CARD
		...
		case 3: //FOR TESTING PURPOSES
				showDeck(deckOfCards_face, size);
				break;

		case 4: //FOR TESTING PURPOSES
				drawCard(deckOfCards_face, deckOfCards_value, size, playerHand);
				break;
		}
	} while (true);
}
...
//FUNCTION DEFINITION FOR CHECKING ALL HANDS FOR A WINNER
void compareHands(int playerHand, int dealerHand) {
	//CHECK FOR BLACKJACK - PLAYER
	if (playerHand == 21 && dealerHand != 21) {
		cout << "\nYou got 21, YOU WIN! Thanks for playing." << endl;
		exitProgram();
	}
	//CHECK FOR BLACKJACK - DEALER
	if (dealerHand == 21 && playerHand != 21) {
		cout << "\nDealer has 21, YOU LOSE! Thanks for playing." << endl;
		exitProgram();
	}
	...
	//CHECK FOR GREATER HAND - PLAYER
	if (dealerHand >= 17 && dealerHand < playerHand && playerHand < 21) {
		cout << "YOU WIN!" << endl;
		cout << endl;
		exitProgram();
	}

	//CHECK FOR GREATER HAND - DEALER
	if (dealerHand >= 17 && dealerHand > playerHand && dealerHand < 21) {
	...
	//CHECK FOR DRAW
	if (playerHand == dealerHand && dealerHand >= 17) {
